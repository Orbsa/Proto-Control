...
pub fn open_output() -> Result<MidiOutputConnection> {
    let midi_out = MidiOutput::new("rotocontrol-out").context("Failed to create MIDI output")?;
...
pub fn open_input() -> Result<(MidiInputConnection<()>, mpsc::Receiver<DeviceEvent>)> {
    let midi_in = MidiInput::new("rotocontrol-in").context("Failed to create MIDI input")?;
        ...
        .connect(
            ...
            move |_timestamp, message, _| {
                ...
                let evt = if cc >= KNOB_CC_BASE && cc < KNOB_CC_BASE + n {
                    DeviceEvent::KnobTurn {
                        index: (cc - KNOB_CC_BASE) as usize,
                        value,
                    }
                } else if cc >= BUTTON_CC_BASE && cc < BUTTON_CC_BASE + n {
                    DeviceEvent::ButtonPress {
                        index: (cc - BUTTON_CC_BASE) as usize,
                        value,
                    }
                } else if cc >= DISCORD_KNOB_CC_BASE && cc < DISCORD_KNOB_CC_BASE + n {
                    DeviceEvent::DiscordKnobTurn {
                        index: (cc - DISCORD_KNOB_CC_BASE) as usize,
                        value,
                    }
                } else if cc >= DISCORD_BUTTON_CC_BASE && cc < DISCORD_BUTTON_CC_BASE + n {
                    DeviceEvent::DiscordButtonPress {
                        index: (cc - DISCORD_BUTTON_CC_BASE) as usize,
                        value,
                    }
                } else if cc >= TS3_KNOB_CC_BASE && cc < TS3_KNOB_CC_BASE + n {
                    DeviceEvent::Ts3KnobTurn {
                        index: (cc - TS3_KNOB_CC_BASE) as usize,
                        value,
                    }
                } else if cc >= TS3_BUTTON_CC_BASE && cc < TS3_BUTTON_CC_BASE + n {
                    DeviceEvent::Ts3ButtonPress {
...
pub fn send_discord_knob_value(
    ...
    conn.send(&[
        0xB0 | MIDI_CHANNEL,
        DISCORD_KNOB_CC_BASE + index as u8,
        value,
    ])
    .map_err(|e| anyhow::anyhow!("Failed to send MIDI: {}", e))
}

pub fn send_discord_button_value(
    conn: &mut MidiOutputConnection,
    index: usize,
    value: u8,
) -> Result<()> {
    ...
    conn.send(&[
        0xB0 | MIDI_CHANNEL,
        DISCORD_BUTTON_CC_BASE + index as u8,
        value,
    ])
    .map_err(|e| anyhow::anyhow!("Failed to send MIDI: {}", e))
}
...
pub fn send_ts3_knob_value(conn: &mut MidiOutputConnection, index: usize, value: u8) -> Result<()> {
    if index >= NUM_CONTROLS {
        bail!("TS3 knob index {} out of range", index);
    }
    conn.send(&[0xB0 | MIDI_CHANNEL, TS3_KNOB_CC_BASE + index as u8, value])
        .map_err(|e| anyhow::anyhow!("Failed to send MIDI: {}", e))
}

pub fn send_ts3_button_value(
    conn: &mut MidiOutputConnection,
    index: usize,
    value: u8,
) -> Result<()> {
